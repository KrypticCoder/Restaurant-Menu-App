...
# JSON object for all restaurants
@app.route('/restaurants/JSON/')
def restaurantsJSON():
    restaurants = session.query(Restaurant).all()
    return jsonify(Restaurants=[i.serialize for i in restaurants])

# JSON object for menu of specific restaurant
...
# JSON object for specific menu item of a restaurant
...
def editRestaurant(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash ("restaurant '" + editRestaurant.name + "' has been edited")
        return redirect(url_for('showRestaurants'))
    else:
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("item '"+ editedItem.name + "' has been edited")
        return redirect(url_for('showMenu', restaurant_id=restaurant_id))
    else:
